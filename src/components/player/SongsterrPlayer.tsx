...
import { ChevronDown, SlidersHorizontal, Sparkles, Music2, Share2, Volume2, Menu } from 'lucide-react';
...
export const SongsterrPlayer: React.FC<SongsterrPlayerProps> = ({
  onOpenStudioTools,
  onOpenTuxGuitar,
}) => {
  // Current Song & Track
  ...
  return (
    <div className="flex flex-col h-full w-full bg-[#0a0c10] text-zinc-100 overflow-hidden relative select-none">
      ...
      <div className="shrink-0 bg-zinc-950/90 backdrop-blur-md border-b border-zinc-800/80 px-3 py-2 z-20 flex items-center justify-between gap-2">
        {/* Left: Menu Hamburger + Song selector chip */}
        <div className="flex items-center space-x-2">
          {onOpenStudioTools && (
            <button
              onClick={onOpenStudioTools}
              className="p-1.5 rounded-xl bg-zinc-900/90 hover:bg-zinc-800 border border-zinc-800 text-amber-400 hover:text-amber-300 transition-colors active:scale-95 cursor-pointer"
              title="Открыть меню студии (Все разделы)"
            >
          ...
          <button
            ...
            className="flex items-center space-x-2 bg-zinc-900/90 hover:bg-zinc-800 border border-zinc-800 px-2.5 py-1.5 rounded-full max-w-[160px] sm:max-w-xs transition-colors active:scale-95"
          >
        ...
        <div className="flex items-center space-x-1.5">
          ...
          <button
            ...
            className="p-1.5 rounded-full bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-zinc-200 active:scale-90 cursor-pointer"
      ...
      <MoreOptionsSheet
        ...
        onOpenTuxGuitar={onOpenTuxGuitar}
      />
