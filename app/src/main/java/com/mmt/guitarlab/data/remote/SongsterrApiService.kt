...
class SongsterrApiService @Inject constructor() {
    ...
    suspend fun fetchSongScore(songId: Int, requestedTrackIndex: Int? = null): Result<TabScore> = withContext(Dispatchers.IO) {
        runCatching {
            ...
            // Find best initial track index if not specified
            val initialTrackIndex = requestedTrackIndex ?: run {
                var bestIdx = 0
                var bestViews = -1
                for (t in 0 until tracksArray.length()) {
            ...
            // Identify playable tracks to fetch concurrently (up to 8 tracks)
            val playableIndices = mutableListOf<Int>()
            // Ensure initialTrackIndex is first
            playableIndices.add(initialTrackIndex)
            for (t in 0 until tracksArray.length()) {
                if (t == initialTrackIndex) continue
                val trk = tracksArray.getJSONObject(t)
                val isVocal = trk.optBoolean("isVocalTrack", false)
                val isEmpty = trk.optBoolean("isEmpty", false)
                if (!isVocal && !isEmpty && playableIndices.size < 8) {
                    playableIndices.add(t)
                }
            }

            // Fetch JSONs in parallel for ultra-fast multi-track loading
            val fetchedTracks = playableIndices.map { tIdx ->
                async {
                    runCatching {
                        val trackJsonString = fetchTrackJsonFromCdn(songId, revisionId, image, tIdx)
                        tIdx to JSONObject(trackJsonString)
                    }.getOrNull()
                }
            }.awaitAll().filterNotNull().toMap()

            // Parse all fetched tracks
            val allParsedTracks = mutableListOf<TabTrack>()
            var primaryBpm = 120
            var primaryNumerator = 4
            var primaryDenominator = 4

            // Order tracks as they appeared in metadata for consistent track order
            for (tIdx in playableIndices) {
                val tJson = fetchedTracks[tIdx] ?: continue
                val parsed = parseSingleTrack(tJson)
                if (tIdx == initialTrackIndex) {
                    primaryBpm = parsed.tempoBpm
                    primaryNumerator = parsed.timeSigNum
                    primaryDenominator = parsed.timeSigDen
                    allParsedTracks.add(0, parsed.track)
                } else {
                    allParsedTracks.add(parsed.track)
                }
            }

            // Fallback: if only initial was parsed or multiple, ensure at least one track
            if (allParsedTracks.isEmpty()) {
                val fallbackJson = JSONObject(fetchTrackJsonFromCdn(songId, revisionId, image, initialTrackIndex))
                val parsed = parseSingleTrack(fallbackJson)
                allParsedTracks.add(parsed.track)
            }

            TabScore(
                id = "$songId-$revisionId",
                title = title,
                artist = artist,
                revisionDate = "Songsterr #$songId",
                tempo = primaryBpm,
                timeSignatureNumerator = primaryNumerator,
                timeSignatureDenominator = primaryDenominator,
                tracks = allParsedTracks
            )
        }
    }

    private data class ParsedTrackResult(
        val track: TabTrack,
        val tempoBpm: Int,
        val timeSigNum: Int,
        val timeSigDen: Int,
    ...
    private fun parseSingleTrack(trackJson: JSONObject): ParsedTrackResult {
        ...
        return ParsedTrackResult(
            track = track,
            tempoBpm = tempoBpm,
            timeSigNum = currentNumerator,
            timeSigDen = currentDenominator
        )
    }
    ...
    private fun detectTuningName(midiNotes: List<Int>): String {
        return when (midiNotes) {
            ...
            listOf(61, 56, 52, 47, 42, 37) -> "Eb Standard"
            listOf(43, 38, 33, 28) -> "Standard Bass"
            listOf(41, 38, 33, 26) -> "Drop D (Bass)"
            listOf(39, 36, 31, 24) -> "Drop C (Bass)"
            else -> if (midiNotes.size == 4) "Bass Tuning" else "Custom Tuning"
        }
    }
}
