...
class TabRepositoryImpl @Inject constructor(
    ...
    override fun getAllProjects(): Flow<List<TabProjectEntity>> {
        return tabProjectDao.getAllProjects()
    }
    ...
    override suspend fun deleteProject(project: TabProjectEntity): Result<Unit> {
        return withContext(Dispatchers.IO) {
            runCatching {
                tabProjectDao.deleteProject(project)
            }
        }
    }
