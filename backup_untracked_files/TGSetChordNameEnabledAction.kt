...
class TGSetChordNameEnabledAction(context: TGContext) : TGActionBase(context, NAME) {
    override fun processAction(context: TGActionContext) {
        val layout = TGSongViewController.getInstance(getContext()).layout
        layout.style = layout.style xor TGLayout.DISPLAY_CHORD_NAME
    }

    companion object {
        const val NAME = "action.view.layout-set-chord-name-enabled"
