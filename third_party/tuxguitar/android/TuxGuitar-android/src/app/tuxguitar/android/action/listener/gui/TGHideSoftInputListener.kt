...
class TGHideSoftInputListener(private val context:TGContext, private val activity:TGActivity):TGEventListener {
 ...
 fun hideSoftInputFromWindow(){ activity.requireActivity().currentFocus?.let { (activity.requireContext().getSystemService(Context.INPUT_METHOD_SERVICE) as InputMethodManager).hideSoftInputFromWindow(it.windowToken,0) } }
