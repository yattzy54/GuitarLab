...
class TGBrowserAddCollectionAction(context: TGContext) : TGActionBase(context, NAME) {
    override fun processAction(actionContext: TGActionContext) {
        try {
            val manager = TGBrowserManager.getInstance(getContext())
            manager.addCollection(actionContext.getAttribute(ATTRIBUTE_COLLECTION))
    ...
    companion object {
        const val NAME = "action.browser.add-collection"
