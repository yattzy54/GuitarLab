...
class TGGoFirstTrackAction(context: TGContext) : TGActionBase(context, NAME) {
 override fun processAction(context: TGActionContext) {
  ...
  val track = getSongManager(context).getFirstTrack(song)
  if (track != null) getEditor().caret.update(track.number)
 }
 companion object { const val NAME = "action.track.go-first" }
}
